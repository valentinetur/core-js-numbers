...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const vectorMultiply = x1 * x2 + y1 * y2;
  const lengthA = Math.sqrt(x1 ** 2 + y1 ** 2);
  const lengthB = Math.sqrt(x2 ** 2 + y2 ** 2);
  const cosAngleResult = vectorMultiply / (lengthA * lengthB);
  return Math.acos(cosAngleResult);
}
...
function toNumber(value, def) {
  ...
  if (typeof numValue === 'number' && Number.isNaN(numValue) === false) {
    result = Number(value);
  }
...
function getSumOfDigits(num) {
  ...
  const sum = arrNumber.reduce((acc, current) => acc + current, 0);
  return sum;
}
...
function isPowerOfTwo(num) {
  let result = false;
  if (Math.log2(num) === Number(Math.log2(num).toFixed())) {
    result = true;
  }
 ...
 * 12345, 7    => '12345.00'
 * 12.345, 4   => '12.35'
 * const newNumber = Number(number.toString().replace('.', ''));
  let result = newNumber / (10 ** (precision - 1));
  return result;
 */
